...
(function(){
  ...
  function initialsFromEmail(email){
    ...
    const parts = name.replace(/[^a-z0-9]+/ig,' ').trim().split(' ');
  ...
  function openMenu(){
    ...
    const first = accountMenu.querySelector('button, a, [tabindex]:not([tabindex="-1"])');
    first && first.focus();
  }
  function closeMenu(){
    if (!accountMenu) return;
  ...
  function toggleMenu(){
    if (!accountMenu) return;
    if (isVisible(accountMenu)) closeMenu(); else openMenu();
  }
  ...
  auth.onAuthStateChanged((user) => {
    ...
    if (signedIn) {
      ...
      if (avatarInitials) avatarInitials.textContent = initialsFromEmail(email);
      if (userEmailEl) userEmailEl.textContent = email || 'Signed in';
      if (userUidEl)   userUidEl.textContent   = user.uid ? `UID: ${user.uid}` : '';
    } else {
      show(loginBtn);
      hide(accountWrapper);
      closeMenu();
  ...
  accountMenuBtn?.addEventListener('click', (e) => { e.stopPropagation(); toggleMenu(); });
  document.addEventListener('click', (e) => {
    if (!accountMenu || !accountWrapper) return;
    if (accountWrapper.contains(e.target)) return;
    closeMenu();
  });
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
  ...
  logoutBtn?.addEventListener('click', async () => {
    try { await signOut(); } finally { closeMenu(); }
  });

  // Delete account (opens modal; actual deletion handled by delete-account.js)
  openDeleteModalBtn?.addEventListener('click', () => {
    closeMenu();
    show(deleteModal);
  });

  // Footer year helper if present
  const yearEl = document.getElementById('year');
  if (yearEl) yearEl.textContent = new Date().getFullYear();
})();
// ===== End Account Menu / Auth UI =====
